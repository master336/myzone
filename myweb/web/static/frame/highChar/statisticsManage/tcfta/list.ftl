...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.main.tcfta")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list.jhtml" method="get">
		<div class="bar">
			${message("tcfta.condition.beginDate")}: <input type="text" id="beginDate" name="beginDate"  value="${(beginDate?string('yyyy-MM-dd'))!}" class="text Wdate"  onfocus="WdatePicker({maxDate: '#F{$dp.$D(\'endDate\')}'});" />
			${message("tcfta.condition.endDate")}: <input type="text" id="endDate" name="endDate" value="${(endDate?string('yyyy-MM-dd'))!}" class="text Wdate"   onfocus="WdatePicker({minDate: '#F{$dp.$D(\'beginDate\')}'});" />
		...
		<div class="input tabContent">
			<table id="listTable" class="list">
				<tr>
					...
					<th>
						<span>${message("tcfta.numberNo")}</span>
					</th>
					<th>
						<span>${message("tcfta.destinationName")}</span>
					</th>
					<th>
						<span>${message("tcfta.amount")}</span>
					</th>
				</tr>
