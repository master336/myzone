...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.main.tqa")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list.jhtml" method="get">
		<div class="bar">
			${message("tqa.condition.beginDate")}: <input type="text" id="beginDate" name="beginDate" class="text Wdate" value="${(beginDate?string('yyyy-MM-dd'))!}" onfocus="WdatePicker({maxDate: '#F{$dp.$D(\'endDate\')}'});" />
			${message("tqa.condition.endDate")}: <input type="text" id="endDate" name="endDate" class="text Wdate" value="${(endDate?string('yyyy-MM-dd'))!}" onfocus="WdatePicker({minDate: '#F{$dp.$D(\'beginDate\')}'});" />
		...
		<div class="input tabContent">
			<table id="listTable" class="list">
				<tr>
					...
					<th>
						<span>${message("tqa.varietiesName")}</span>
					</th>
					<th>
						<span>${message("tqa.totalQuantity")}</span>
					</th>
					<th>
						<span>${message("tqa.totalTradingAmount")}</span>
					</th>
				</tr>
