		...
		$("#a-tabs-2").click(function(){
			...
			$.ajax({	
		    	...
		    	success:function(data){
		    		rotationColumnChart('container',data.title,data.subtitle,'笔',data.yAxisTitle,data.xAxis,data.series,'upSlider',0,50,15,'rightSlider',0,50,15);
	   			}
			});
		});
	});
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.main.petn")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list.jhtml" method="get">
		<div class="bar">
			${message("petq.name")}: <input type="text" id="name" name="name" value="${(name)!""}" />
		...
		<div class="input tabContent">
			<table id="listTable" class="list">
				<tr>
					...
					<th>
						<span>${message("petn.numberNo")}</span>
					</th>
					<th>
						<span>${message("petn.name")}</span>
					</th>
					<th>
						<span>${message("petn.num")}</span>
					</th>
				</tr>
