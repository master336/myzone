		...
		$("#a-tabs-2").click(function(){
			...
			$.ajax({	
		    	...
		    	success:function(data){
		    		rotationColumnChart('container',data.title,data.subtitle,'K',data.yAxisTitle,data.xAxis,data.series,'upSlider',0,50,15,'rightSlider',0,50,15);
	   			}
			});
		});
	});
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.main.pfs")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list.jhtml" method="get">
		<div class="bar">
			${message("pfs.condition.productName")}: <input type="text" id="productName" name="productName" value="${(productName)!""}" />
		...
		<div class="input tabContent">
			<table id="listTable" class="list">
				<tr>
					...
					<th>
						<span>${message("pfs.numberNo")}</span>
					</th>
					<th>
						<span>${message("pfs.place")}</span>
					</th>
					<th>
						<span>${message("pfs.num")}</span>
					</th>
				</tr>
